-- Mejorar función para limpiar guiones y caracteres especiales
...
BEGIN
  ...
  LOOP
    -- Limpiar texto: convertir a minúsculas, eliminar guiones, normalizar espacios
    clean_name := LOWER(TRIM(product_record.name));
    clean_name := REGEXP_REPLACE(clean_name, '[_\-]+', ' ', 'g'); -- Reemplazar guiones por espacios
    clean_name := REGEXP_REPLACE(clean_name, '\s+', ' ', 'g'); -- Normalizar espacios múltiples
    ...
    IF word_count >= 2 THEN
      FOR i IN 1..(LEAST(word_count - 1, 3)) LOOP -- Limitar a primeras 3 combinaciones
        IF NOT (words[i] = ANY(stop_words)) AND NOT (words[i+1] = ANY(stop_words)) THEN
          ...
          IF LENGTH(bigram) >= 8 AND NOT (bigram = ANY(keywords_array)) THEN
            ...
            EXIT WHEN array_length(keywords_array, 1) >= 4; -- Máximo 4 keywords hasta ahora (1 primary + 3 bigramas)
          END IF;
        END IF;
      END LOOP;
    END IF;
    
    -- 3. Keyword de Categoría + Primera palabra significativa del producto (solo si aún no tenemos 5)
    IF array_length(keywords_array, 1) < 5 AND category_name != '' AND LENGTH(category_name) >= 3 THEN
