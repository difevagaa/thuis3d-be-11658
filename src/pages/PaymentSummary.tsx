      ...
      try {
        ...
        try {
          await supabase.from("invoices").insert({
            ...
            notes: t('payment:messages.autoGeneratedInvoice', { orderNumber: order.order_number })
          });
        } catch (invoiceError) {
        ...
        try {
          await supabase.functions.invoke('send-admin-notification', {
            body: {
              to: 'admin@thuis3d.be',
              type: 'order',
              subject: t('payment:messages.newGiftCardOrder', { orderNumber: order.order_number }),
              message: t('payment:messages.giftCardOrderNotification', { customerName: shippingInfo.full_name || 'Cliente' }),
          ...
          if (adminProfiles && adminProfiles.length > 0) {
            for (const admin of adminProfiles) {
              await supabase.rpc('send_notification', {
                p_user_id: admin.id,
                p_type: 'new_order',
                p_title: t('payment:messages.newGiftCardOrder', { orderNumber: order.order_number }),
                p_message: t('payment:messages.giftCardOrderAdminNotification', { customerName: shippingInfo.full_name || 'Cliente', total: '0.00' }),
