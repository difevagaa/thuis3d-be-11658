...
export default function MyAccount() {
  ...
  const loadUserData = async (userId: string) => {
    try {
      ...
      // Fetch coupon details separately for each redemption that has a coupon_code
      const couponCodes = redemptionsRes.data?.map(r => r.coupon_code).filter(Boolean) || [];
      ...
      if (couponCodes.length > 0) {
        const { data: couponDetails } = await supabase
          ...
          .in("code", couponCodes);
        if (couponDetails) {
          couponDetails.forEach(c => { couponMap[c.code] = c; });
        }
      }
      ...
      const userCoupons = redemptionsRes.data?.map(redemption => ({
        ...
        discount_type: couponMap[redemption.coupon_code || '']?.discount_type || 'percentage',
        discount_value: couponMap[redemption.coupon_code || '']?.discount_value || 0,
